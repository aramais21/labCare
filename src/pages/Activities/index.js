...
const Activities = () => {
    ...
    return(
        <ActivityWrapper>
            {data.map((item, index) => {
                return (
                    <ActivityGridComponent key = {item.id} padding = '71px' startHeight = {index<=1?index+1:index+2} endHeight = {index<=1?index+3:index+4}  backgroundColor = {item.backgroundColor} startWidth = {index%2?1:2} endWidth = {index%2?2:3} >
            ...
            {images.map((item, index) => {
                return (
                    <ActivityGridComponent key = {item.id} startHeight = {index<=1?index%2?index+2:index+1:index%2?index+3:index+2} endHeight = {index<=1?index%2?index+3:index+2:index%2?index+4:index+3}  startWidth = {index%2?2:1} endWidth = {index%2?3:2}>
                        <Image src = {item.src} noMargin = {true} width = '50vw ' height = '50vh ' />
                    </ActivityGridComponent>
                )
            })}
        </ActivityWrapper>
